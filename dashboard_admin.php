...
// 2. Captura o filtro da URL (Se for vazio ou 'geral', mostra todas)
$fabrica_filtro = isset($_GET['fabrica']) && is_numeric($_GET['fabrica']) ? (int)$_GET['fabrica'] : null;

// 3. Monta a "peça de Lego" do SQL que será injetada em TODAS as suas queries
$where_fabrica = "";
$params_query = [];

if ($fabrica_filtro) {
    // Se escolheu uma fábrica, o SQL vai exigir que a linha pertença a ela
    $where_fabrica = " AND l.fabrica = :fabrica_id ";
    $params_query[':fabrica_id'] = $fabrica_filtro;
}

// Define as variáveis de filtro dinâmico
$sql_fabrica = "";
$param_fabrica = [];
if (!empty($fabrica_filtro)) {
...
try {
    // ========================================================================
    // BLOCO 1: DADOS GERAIS (HISTÓRICO DA EMPRESA OU FÁBRICA)
    // ========================================================================
    ...
    $stmt_oee = $pdo->prepare("
        ...
        WHERE a.oee_geral > 0 {$sql_fabrica}
    ");
    $stmt_oee->execute($param_fabrica);
    ...
    $stmt_totais = $pdo->prepare("
        ...
        JOIN linhas l ON a.linha_id = l.id 
        WHERE 1=1 {$sql_fabrica}
    ");
    $stmt_totais->execute($param_fabrica);
    ...
    ");
    $stmt_linhas->execute($param_fabrica);
    ...
    $stmt_produtos = $pdo->prepare("
        SELECT p.descricao, SUM(ap.producao_boas) as qtd_total 
        FROM apontamento_producao ap 
    ...
    ");
    $stmt_produtos->execute($param_fabrica);
    ...
    // ========================================================================
    // BLOCO 2: DADOS MENSAIS (FILTRADOS POR MÊS, ANO E FÁBRICA)
    // ========================================================================
    
    // Mescla os parâmetros de mês/ano com o filtro de fábrica, se existir
    $params_mensal = [$mes_filtro, $ano_filtro];
    if (!empty($fabrica_filtro)) {
        $params_mensal[] = $fabrica_filtro;
    }
...
<body class="bg-slate-50 min-h-screen font-sans pb-12 text-slate-800">
    ...
    <div class="flex flex-wrap gap-2 border-b border-slate-200 pb-4 mb-6 mt-4">
        <!-- Botão Visão Geral -->
        <a href="dashboard_admin.php" class="px-5 py-2.5 rounded-lg text-sm font-bold transition-colors <?= !$fabrica_filtro ? 'bg-slate-800 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-100' ?>">
            Visão Global (Todas)
        </a>

        <!-- Botões Dinâmicos das Fábricas -->
        <?php foreach ($lista_fabricas as $fab): ?>
            <a href="?fabrica=<?= $fab ?>" class="px-5 py-2.5 rounded-lg text-sm font-bold transition-colors <?= $fabrica_filtro === $fab ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-100' ?>">
                Fábrica <?= $fab ?>
            </a>
        <?php endforeach; ?>
    </div>
    ...
    <script>
        ...
        new Chart(document.getElementById('graficoOEE').getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    label: 'OEE (%)',
                    data: <?= json_encode($data_oee_linhas) ?>,
                    backgroundColor: <?= json_encode($cores_fabricas) ?>,
                    borderRadius: 4,
                    order: 2
                }, {
                    label: 'Meta Mundial (85%)',
                    data: Array(<?= count($labels_linhas) ?>).fill(85),
                    type: 'line',
